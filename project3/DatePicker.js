...
DatePicker.prototype.render = function (date) {
  var i, j;
  var num2weekday = ["Ня", "Да", "Мя", "Лха", "Пү", "Ба", "Бя"];
  var num2month = [
    "January",
    "Febuary",
    "March",
  ...
  function getMonthDays(thisDate) {
    ...
    var iterDate;
    dateStartingPoint.setDate(
      dateStartingPoint.getDate() - dateStartingPoint.getDay()
    );
      ...
      for (j = 0; j < oneWeekDays.length; j++) {
        ...
        if (oneWeekDays[j].isCurrentMonth) {
          ...
          (function (selectedDate, daysInAMonthDiv) {
            daysInAMonthDiv.addEventListener(
              "click",
              function () {
                var arrOfAllDates = document
                  .getElementById(self.id)
                  .getElementsByClassName(daysInAMonthClassName);
